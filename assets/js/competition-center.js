(function(){
  ...
  const yearLabels={'2022':'2022 - Ozarks','2023':'2023 - Mesquite','2024':'2024 - Myrtle Beach','2025':'2025 - Kohler'};
  const flourishIds={'2022':'29756767','2023':'13801317','2024':'29756784','2025':'29756817'};
  ...
  function playerImage(name){const p=data.players[name]||data.players[name.replace('Santomauro','Santomauro')];return p?'../assets/images/'+p.image:'../assets/logos/trophy.png'}
  function profileLink(name){const p=data.players[name]||data.players[name.replace('Santomauro','Santomauro')];return p?'../players/'+p.profile:'#'}
  function updateUrl(){history.replaceState(null,'',`?year=${activeYear}&session=${activeSession}`)}
  function renderYears(){
    $('yearNav').innerHTML=years.map(y=>`<button class="year-button ${y===activeYear?'active':''}" data-year="${y}">${esc(yearLabels[y]||y)}</button>`).join('');
  ...
  function teamRoster(t){return `<div class="board-roster">${t.players.map(p=>`<a href="${profileLink(p.name)}" title="${esc(p.name)}"><img src="${playerImage(p.name)}" alt=""><span>${esc(p.name)}</span></a>`).join('')}</div>`}
  function renderBoard(){
    const y=data.years[activeYear],a=y.teams[0],b=y.teams[1],sa=y.overallScores[a.id]||0,sb=y.overallScores[b.id]||0;
    const target=(Math.floor((y.totalPointsAvailable/2)*2)/2)+0.5;
    $('overallScoreboard').innerHTML=`<section class="overall-board"><div class="board-head"><div class="team-score"><div class="board-kicker">${esc(activeYear)} • ${esc(y.location)}</div><div class="team-name" style="color:${a.color}">${esc(a.name)}</div><div class="team-points" style="color:${a.color}">${fmt(sa)}</div>${teamRoster(a)}</div><div class="score-divider"><span>OVERALL</span><strong>${fmt(target)} to win</strong></div><div class="team-score"><div class="board-kicker">Competition Score</div><div class="team-name" style="color:${b.color}">${esc(b.name)}</div><div class="team-points" style="color:${b.color}">${fmt(sb)}</div>${teamRoster(b)}</div></div><div class="score-breakdown"><div><strong>${fmt(y.matchScores[a.id]||0)} – ${fmt(y.matchScores[b.id]||0)}</strong><span>Match points</span></div><div><strong>${fmt(y.netScores[a.id]||0)} – ${fmt(y.netScores[b.id]||0)}</strong><span>Net Championship points</span></div><div><strong>${fmt(y.totalPointsAvailable)}</strong><span>Total points available</span></div></div></section>${y.historicalNote?`<p class="historical-note">${y.historicalNote}</p>`:''}`;
  }
  ...
  function sessionTeamScores(y,s){
    ...
    if(s.type==='net'){(s.pointsAwards||s.podium).forEach(p=>{if(p.teamId)scores[p.teamId]+=p.points})} else if(s.type==='special'){Object.entries(s.scores||{}).forEach(([id,v])=>scores[id]+=v)}
  ...
  function renderNet(y,s){
    const podium=s.podium.map(p=>{const t=team(y,p.teamId);return `<div class="podium-place"><span class="place-number">${esc(p.placeLabel||p.place)}</span><img src="${playerImage(p.player)}" alt="${esc(p.player)}"><h3>${esc(p.player)}</h3><p>${esc(t.name)}</p><p>Net ${esc(p.net)} • Gross ${esc(p.gross)}</p><div class="podium-points">${Number(p.points)>0?'+':''}${fmt(p.points)} pts</div></div>`}).join('');
    ...
    const chartId=flourishIds[String(y.year)];
    const chart=chartId?`<div class="net-chart"><h3>Championship Progression</h3><iframe title="${esc(y.year)} BGW Net Championship progression" src="https://flo.uri.sh/visualisation/${chartId}/embed" loading="lazy" frameborder="0" scrolling="no"></iframe></div>`:'';
    return `<section class="net-card"><div class="podium">${podium}</div><div class="hole-scroll"><table class="standings-table"><thead><tr><th>Pos.</th><th>Player</th><th>Team</th><th>Gross</th><th>Net</th></tr></thead><tbody>${rows}</tbody></table></div>${chart}</section>`
  }
