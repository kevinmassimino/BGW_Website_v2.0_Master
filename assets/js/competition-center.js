(function(){
  ...
  function renderBoard(){
    ...
    const target=Math.floor(y.totalPointsAvailable/2)+1;
  ...
  function sessionTeamScores(y,s){
    ...
    if(s.type==='net'){s.podium.forEach(p=>{if(p.teamId)scores[p.teamId]+=p.points})}
  ...
  function renderSession(){
    ...
    $('matchList').innerHTML=s.type==='net'?renderNet(y,s):s.matches.map(m=>renderMatch(y,m)).join('');
  ...
  function renderMatch(y,m){
    ...
    return `<article class="match-card"><div class="match-summary"><div class="match-side">${avatars(m.sideA)}<div><div class="side-label" style="color:${ta.color}">${esc(m.sideA)}</div><small>${esc(ta.name)}</small></div></div><div class="match-result"><span>${esc(m.format)}</span><strong>${esc(m.result)}</strong><span>${aWon?esc(m.sideA):bWon?esc(m.sideB):'Halved'}</span><button type="button">Hole-by-hole ▾</button></div><div class="match-side side-b"><div><div class="side-label" style="color:${tb.color}">${esc(m.sideB)}</div><small>${esc(tb.name)}</small></div>${avatars(m.sideB)}</div></div><div class="match-details"><div class="hole-scroll"><div class="hole-grid">${holes}</div></div><p class="details-note">Scores shown are the handicap-adjusted values supplied in the BGW Match Holes data. No handicap calculation is performed on this page.</p></div></article>`
  }
