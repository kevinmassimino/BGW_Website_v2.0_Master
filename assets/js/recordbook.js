(() => {
  ...
  const bundledData = window.BGW_RECORDS_DATA;
  if (bundledData) {
    state.data = bundledData;
    buildControls();
    render();
  } else {
    content.innerHTML = `<div class="record-error"><h2>Record book unavailable</h2><p>The bundled record data could not be loaded.</p></div>`;
  }
